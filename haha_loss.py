    ...
    def prepare_targets(self, targets, anchors):
        ...
        for im_i in range(len(targets)):
            ...
            if self.cfg.MODEL.ATSS.POSITIVE_TYPE == 'SSC':
                ...
                # Selecting candidates based on the DIoU between the anchor box and object
                candidate_idxs = []
                star_idx = 0
                num_anchors_per_level = [len(anchors_per_level.bbox) for anchors_per_level in anchors[im_i]]
                for level, anchors_per_level in enumerate(anchors[im_i]):
                ...
                is_in_gts = torch.stack([l, t, r, b], dim=1).min(dim=1)[0] > 0.01
                is_pos = is_pos & is_in_gts
                # print('is_pos_size', is_pos.size())

                # if an anchor box is assigned to multiple gts, the one with the highest DIoU will be selected.
                dious_inf = torch.full_like(dious, -INF).t().contiguous().view(-1)
                index = candidate_idxs.view(-1)[is_pos.view(-1)]
                dious_inf[index] = dious.t().contiguous().view(-1)[index]
                dious_inf = dious_inf.view(num_gt, -1).t()

                anchors_to_gt_values, anchors_to_gt_indexs = dious_inf.max(dim=1)
                cls_labels_per_im = labels_per_im[anchors_to_gt_indexs]
                cls_labels_per_im[anchors_to_gt_values == -INF] = 0
                matched_gts = bboxes_per_im[anchors_to_gt_indexs]
